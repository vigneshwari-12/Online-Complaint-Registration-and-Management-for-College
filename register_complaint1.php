...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($type_block == "--select--" || $type_complaint == "--select--" || $type_priority == "--select--") {
        ...
        if(isset($_FILES['file']) && $_FILES['file']['error'] === UPLOAD_ERR_OK) {
            ...
            // If no file uploaded or error occurred, set file data to null or handle error as per your requirement
            $file_data = null;
            // You might want to display an error message or log the error
        }
        // Insert data into database
        $sql = "INSERT INTO complaint_details(Student_id,query_id,block_issue,complaint_type,priority,details_of_issue,file_data) VALUES ('$username', '$complaint_id','$type_block', '$type_complaint', '$type_priority', '$complaint','$file_data')";
